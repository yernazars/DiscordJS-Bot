...
module.exports = {
    name: 'jump',
    description:("Jumps to specific track in queue"),
    voiceChannel: true,
    options: [
        {
            name: 'song',
    ...
    async execute({ inter }) {
        ...
        if (!track && !number) inter.editReply({ content: await Translate(`You have to use one of the options to jump to a song <${inter.member}><❌>`) });

        let trackName;
        if (track) {
            const toJump = queue.tracks.toArray().find((t) => t.title.toLowerCase() === track.toLowerCase() || t.url === track);
            if (!toJump) return inter.editReply({ content: await Translate(`could not find <${track}> <${inter.member}><❌>`) });
            ...
            if (!name) return inter.editReply({ content: await Translate(`This track does not exist <${inter.member}><❌>`) });
