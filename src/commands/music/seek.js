...
module.exports = {
    ...
    async execute({ inter }) {
        ...
        if (!queue?.isPlaying()) return inter.editReply({ content: await Translate(`No music currently playing <${inter.editReply}><❌>`) });

        const timeToMS = ms(inter.options.getString('time'));
        if (timeToMS >= queue.currentTrack.durationMS) {
            return inter.editReply({ content: await Translate(`The indicated time is higher than the total time of the current song <${inter.member}><❌\n> *Try for example a valid time like <**5s, 10s, 20 seconds, 1m**>...*`) });
        }
