...
module.exports = {
    name: 'playnext',
    description:("Play a song after the current one"),
    voiceChannel: true,
    options: [
        {
            name: 'song',
    ...
    async execute({ inter }) {
        ...
        if (res.playlist) return inter.editReply({ content: await Translate(`This command dose not support playlist's <${inter.member}><❌>`) });

        queue.insertTrack(res.tracks[0], 0);

        const playNextEmbed = new EmbedBuilder()
            .setAuthor({ name: await Translate(`Track has been added to the queue <🎧>`) })
            .setColor('#2f3136');
